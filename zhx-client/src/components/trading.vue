<template>
  <div class='container'>
        ...
        <li v-for="order in dataList" :key="order.order_item_id">
          <div>
            <div class="top">
              ...
              <i v-if="order.order_status == 1" class="orderState" style="background-color:lightskyblue">已发货</i>
            ...
            <div class="buttom">
              <div class="time">
                ...
                <span v-if="order.order_status == 2 && order.send_time">发货时间：{{ order.send_time }}</span>
                <span v-if="order.order_status == -1">退货时间：{{ order.update_time }}</span>
                <span v-if="order.order_status == -1">退货理由：{{ order.return_info }}</span>
                <span v-if="order.order_status == -2 && order.return_status == 1">拒绝理由：{{ order.return_info }}</span>
                <span v-if="order.order_status == 2 && order.user_evaluation">买家评论：{{ order.user_evaluation }}</span>
                <span v-if="order.order_status == 2 && order.boss_evaluation">我的评论：{{ order.boss_evaluation }}</span>
              </div>
              ...
              <div class="btn">
                ...
                <div class="button" v-if="order.order_status == 2">
                  <van-button size="small" color="#ffb6ce" v-if="!Boolean(order.boss_evaluation)"
                    @click="evaluationBtn(order)">评价</van-button>
                </div>
              </div>
    ...
    <!-- 拒绝退货弹窗 -->
    <van-dialog v-model="showDisAgreeDialog" title="拒绝退货信息填写" show-cancel-button :before-close="onBeforeClose"
      @confirm="disAgreeConfirm">
      <van-form ref="disAgreeform">
        <van-field v-model="checkOrder.close_info" name="message" label="拒绝理由:" rows="2" autosize type="textarea"
          maxlength="50" show-word-limit placeholder="请输入拒绝理由" :rules="[
    ...
    <van-dialog v-model="showEvaluationDialog" title="评价信息填写" show-cancel-button :before-close="onBeforeClose"
      @confirm="evaluationfirm">
      <van-form ref="evaluationform">
        <van-field v-model="checkOrder.boss_evaluation" name="message" label="评价:" rows="2" autosize type="textarea"
          maxlength="50" show-word-limit placeholder="请输入你的评价" :rules="[
            { required: true, message: '请输入你的评价' }]" />
      </van-form>
    </van-dialog>
  </div>
</template>
...
export default {
  ...
  computed: {
    title() {
      return this.$route.query.state == 'complete' ? '交易完成' : '交易中'
    }
  },
  ...
  methods: {
    ...
    // 确认拒绝退货
    disAgreeConfirm() {
      this.$refs.disAgreeform.validate()
    ...
    onBeforeClose(action, done) {
      if (action === "confirm") {
        ...
        this.$refs.disAgreeform && this.$refs.disAgreeform.resetValidation("message");
    ...
    getTradingOrder() {
      this.dataList = []
      let orderStatus = this.$route.query.state == 'complete' ? [-2, 2] : [-1, 0, 1]  //查找退货中,待发货,待收货的订单
        ...
        service._getOrderItemByBossId(data).then(res => {
          if (res.status === 200 && res.data.length != 0) {
            res.data.forEach(item => {
              // let obj = {
              //   ...item,
              //   create_time:this.getDate(item.create_time),
              //   update_time:this.getDate(item.update_time),
              //   create_time:this.getDate(item.create_time),
              // }
